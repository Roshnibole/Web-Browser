...
/**
 * This package-private class defines one Sk_browser window for use with 
...
class BrowserWindow extends Stage {
     ...
     * @param Sk_browser     the WebBrowser application that is createing this window
     * @param Sk_initialURL  if non-null, the window will attempt to load this URL
     */
    BrowserWindow(WebBrowser browser, String initialURL) {
        ...
        ebEngine.titleProperty().addListener( (o,oldVal,newVal) -> {
            if (newVal == null)
                setTitle("Untitled " + Sk_owner.getNextUntitledCount());
            else
                setTitle(newVal); 
        });
        ...
        cancelButton.setOnAction( e -> { // Cancel the ongoing page load
            if ( Sk_webEngine.getLoadWorker().getState() == Worker.State.RUNNING )
                Sk_webEngine.getLoadWorker().cancel();
       ...
       webEngine.setConfirmHandler( str -> 
        ...
        /* If the Sk_initialURL is not null, load that page! */
    ...
    private MenuBar makeMenuBar() {
        ...
        newWin.setOnAction( e -> owner.Sk_newBrowserWindow(null) );
    ...
    private void populateWindowMenu() {
        ArrayList<BrowserWindow> windows = Sk_owner.getOpenWindowList();
